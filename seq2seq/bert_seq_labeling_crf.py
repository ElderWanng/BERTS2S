...
from seq2seq.BS2S_tokenizer import Tokenizer
from seq2seq.model.crf import CRFLayer
...
class BertSeqLabelingCRF(BasicBert):
    ...
    def __init__(self, word2ix, target_size, model_name="roberta"):
        ...
        if model_name == "roberta":
            from seq2seq.model.roberta_model import BertModel, BertConfig, BertPredictionHeadTransform
            config = BertConfig(len(word2ix))
            self.bert = BertModel(config)
            self.transform = BertPredictionHeadTransform(config)
        elif model_name == "bert":
            from seq2seq.model.bert_model import BertConfig, BertModel, BertPredictionHeadTransform
            config = BertConfig(len(word2ix))
            self.bert = BertModel(config)
            self.transform = BertPredictionHeadTransform(config)
        else :
